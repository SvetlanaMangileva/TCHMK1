...
LongNumber SUB(LongNumber a, LongNumber b)
{
	...
	for (; i<a.size; i++) 
	{
		tmp = (long long int) a.pointer[i] - (long long int) carry;
...
LongNumber MUL(LongNumber a, LongNumber b)
{
    ...
    for (i = 0; i < b.size; i++)
    {
        ...
        for (j = 0; j < a.size; j++)
        {
            tmp = (unsigned long long int) b.pointer[i] * (unsigned long long int) a.pointer[j] + (unsigned long long int) carry + (unsigned long long int) res.pointer[i + j];
            carry = tmp / BASE;
...
LongNumber SmallMul(LongNumber a, unsigned long long int b)
{
	...
	for (j = 0; j < a.size; j++)
	{
		tmp = b * (unsigned long long int) a.pointer[j] + (unsigned long long int) carry + (unsigned long long int) res.pointer[j];
		carry = tmp / BASE;
...
LongNumber SmallDIV(LongNumber a, unsigned long long int b)
{
        ...
        {	
            tmp = (unsigned long long int) carry * BASE + (unsigned long long int) a.pointer[i];
            res.pointer[i] = tmp / b;
